...
import {reduxSignOut, reduxSignIn} from "../../src/redux-features/user/userSlice";

const Index = () => {

    const router = useRouter()
    const oobCode = router.query.oobCode || "empty"
    ...
    const handleResetPassword = (event) => {
        ...
        if (passwordValue === confirmPass && confirmPass !== "" && passwordValue !== "") {
            confirmPasswordReset(auth, oobCode, passwordValue).then(response => {
                dispatch(reduxSignIn({}))
            ...
            setError({status: true, message: "Password Not Match!"})
        }
    ...
    return (<m.div
        ...
        <div
            className={'grid grid-cols-12 grid-flow-col max-w-screen-laptop mx-auto items-center laptop:items-start w-full'}>
            ...
            <div className={'col-span-12 md:col-span-6 p-4 tablet:ml-12 tablet:my-12 tablet:max-w-screen'}>
                <div className={'w-full flex flex-col items-center justify-center relative'}>
                    ...
                    {loading === true && (
                        <div
                            className={'flex flex-row justify-center gap-4 items-center w-full h-full absolute bg-skin-theme-body-200/60 animate-pulse'}>
                    ...
                    <m.div variants={animationStore.staggerShort}
                        ...
                        <m.h2
                            ...
                            className={'text-2xl font-bold text-skin-theme-font-100'}>Reset your password!
                        </m.h2>
                        ...
                        <form onSubmit={handleResetPassword}>
                            <div className={'flex flex-col gap-5 justify-start'}>
                                ...
                                <m.button
                                    variants={animationStore.loadOpacityWithYAngle}
                                    className={'rounded-lg w-full py-3 text-lg font-semibold bg-gradient-to-tl from-skin-theme-400 to-skin-theme-500 border border-skin-theme-50 text-skin-theme-font-900'}>Reset
                                    Password
                                </m.button>
                                {error.status && (
                                    <p className={'w-full py-2 text-center text-sm text-skin-theme-font-50 font-semibold bg-skin-theme-body-800/20 rounded'}> {error.message}</p>)}
                            </div>
                        </form>
                    </m.div>

                    <p className={'text-[0.65rem] text-orange-500 mt-4'}>{oobCode}</p>
                </div>
            </div>
        </div>
    </m.div>)
